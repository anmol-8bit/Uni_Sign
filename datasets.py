...
# Scheduled augmentations (train-only); file name must match: datasets_augmentations.py
from datasets_augmentations import apply_pose_augmentations
...
def crop_scale(motion, thr):
    ...
    xmin = float(np.min(valid_coords[:, 0]))
    xmax = float(np.max(valid_coords[:, 0]))
    ymin = float(np.min(valid_coords[:, 1]))
    ymax = float(np.max(valid_coords[:, 1]))
    ratio = 1.0
...
def bbox_4hands(left_keypoints, right_keypoints, hw):
    """
    Return boxes for left/right hands across frames and the (square) half-width.
    """
    ...
    left_box_hw = max(W * max(left_diff_x), H * max(left_diff_y))
    ...
    right_box_hw = max(W * max(right_diff_x), H * max(right_diff_y))

    box_hw = int(max(left_box_hw, right_box_hw) * 1.2 / 2) * 2
    box_hw = max(box_hw, 0)

    left_new_box = np.stack([left_mean_x - box_hw / 2, left_mean_y - box_hw / 2,
                             left_mean_x + box_hw / 2, left_mean_y + box_hw / 2]).astype(np.int16)
    right_new_box = np.stack([right_mean_x - box_hw / 2, right_mean_y - box_hw / 2,
                              right_mean_x + box_hw / 2, right_mean_y + box_hw / 2]).astype(np.int16)
...
def load_support_rgb_dict(tmp, skeletons, confs, full_path, data_transform):
    ...
    if len(left_confs_filter_indices) == 0:
        ...
        left_confs_sel = confs[left_confs_filter_indices]
        left_confs_sel = left_confs_sel[:, 0, [95, 99, 103, 107, 111]].min(-1)
        left_weights = np.max(left_confs_sel) - left_confs_sel + 1e-5
    ...
    if len(right_confs_filter_indices) == 0:
        ...
        right_confs_sel = confs[right_confs_filter_indices]
        right_confs_sel = right_confs_sel[:, 0, [95 + 21, 99 + 21, 103 + 21, 107 + 21, 111 + 21]].min(-1)
        right_weights = np.max(right_confs_sel) - right_confs_sel + 1e-5
    ...
    sampled_indices = np.unique(np.concatenate(all_indices))
    ...
    if left_sampled_indices is None:
        ...
        left_skeletons_norm = (left_skeletons_norm / box_hw).clip(0, 1)
    ...
    if right_sampled_indices is None:
        ...
        right_skeletons_norm = (right_skeletons_norm / box_hw).clip(0, 1)
    ...
    def collate_fn(self, batch):
        ...
        for key in keys:
            ...
            padded_video = [torch.cat(
                (vid[key], vid[key][-1][None].expand(max_len - len(vid[key]), -1, -1),), dim=0
            ) for vid in pose_tmp]
...
# -----------------------------
# CSL_Daily / WLASL dataset (unchanged sampling, no aug scheduling)
# -----------------------------
class S2T_Dataset(Base_Dataset):
    ...
    def load_pose(self, path):
        ...
        confs = pose['scores']
        skeletons = [skeletons[idx] for idx in tmp]
        confs = [confs[idx] for idx in tmp]
...
# -----------------------------
# CSL_News dataset (scheduled augmentations for train)
# -----------------------------
class S2T_Dataset_news(Base_Dataset):
    ...
    def set_augmentation_progress(self, progress: float):
        self.aug_progress = float(max(0.0, min(1.0, progress)))
    ...
    def __getitem__(self, index):
        ...
        for _ in range(num_retries):
            ...
            try:
                ...
                import traceback
                traceback.print_exc()
                print(f"Failed to load video: {name_sample}. Resampling...")
    ...
    def load_pose(self, pose_name, rgb_name):
        ...
        skeletons_all = pose['keypoints']
        confs_all = pose['scores']
        skeletons = [skeletons_all[idx] for idx in tmp]
        confs = [confs_all[idx] for idx in tmp]

        # Train-only scheduled augmentations (no change to dataset length)
        if self.phase == 'train':
            tmp, skeletons, confs = apply_pose_augmentations(
                tmp, skeletons, confs, duration, self.phase, progress=self.aug_progress
            )
